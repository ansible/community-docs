...
Besides container artifacts, it will contain a ``Containerfile`` generated by ``ansbible-builder``,
our inventories and playbooks we will add later to test the EE with ``ansible-runner``.

6. Build the EE container image called ``postgresql_ee`` using the ``demo`` directory as a build context:
